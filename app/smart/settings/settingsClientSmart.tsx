...
export default function SettingsClientSmart({
  scope,
  initialRows,
}: {
  scope: string;
  initialRows: SettingsRow[];
}) {
  const [showHidden, setShowHidden] = useState(false);

  const [rows, setRows] = useState<SettingsRow[]>(
    (initialRows ?? []).map((r) => ({
      scope: s(r.scope) || scope,
      metric_name: s(r.metric_name),
      label: r.label ?? s(r.metric_name),
      kpi_name: r.kpi_name ?? r.label ?? s(r.metric_name),
      enabled: !!r.enabled,
      hidden: !!r.hidden,
  ...
  function updateRow(metric_name: string, patch: Partial<SettingsRow>) {
  ...
  return (
    <div style={{ border: "1px solid #ddd", borderRadius: 14, overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.85 }}>
            Enabled weight sum:{" "}
            <span style={{ fontVariantNumeric: "tabular-nums" }}>{enabledWeightSum.toFixed(2)}</span> (target: 1.00)
          </div>
          ...
          <tbody>
            {visibleRows.map((r) => (
              <tr key={r.metric_name} style={r.hidden ? { opacity: 0.6 } : undefined}>
                ...
                <td style={td}>
                  <select
                    value={(r.format === "percent" ? "percent" : "number") as any}
      ...
      {msg ? <div style={{ padding: 12, borderTop: "1px solid #ddd", fontSize: 12, opacity: 0.9 }}>{msg}</div> : null}
    </div>
  );
}
